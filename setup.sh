...
install_packages() {
    ...
    # Added git-lfs as it's often useful, and ensured unzip is definitely there for bun
...
setup_gh() {
    ...
    # Package is installed in install_packages()

    # Authenticate and configure for the user
    local gh_logic='
        set -euo pipefail
        echo "Authenticating GitHub CLI..."
        
        # HACK: The gh CLI fails if GITHUB_TOKEN is in the environment when you try to
        # log in with a token via stdin. It sees the env var and aborts the stdin
        # method. By unsetting it for just this command, we force it to read from stdin,
        # which correctly stores the credential for future use.
        # We pass the token in a differently named var to avoid the conflict.
    ...
    '
    # Pass GITHUB_TOKEN as GH_CLI_TOKEN to be used inside the subshell logic
